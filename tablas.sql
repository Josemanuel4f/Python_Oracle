CREATE TABLE clinica(
    ...
    CONSTRAINT may_dir CHECK(direccion=initcap (direccion))
);
...
CREATE TABLE propietarios(
    ...
    CONSTRAINT dni_prop CHECK (REGEXP_LIKE(DNI, '[0-9]{8}[A-Z]{1}')),
    constraint nom_prop CHECK (nombre=initcap (nombre)),
    constraint telf_prop CHECK (REGEXP_LIKE(telefono, '^6'))
);
